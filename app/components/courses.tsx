import React from "react";
import Image from "next/image";

const CoursesCards = () => {
  return (
    <div className="w-[1280px] h-[1742px] ">
      <section className="flex flex-col w-[1280px] h-[1742px] gap-[60px] bg-[#ffffff] mt-36">
        <div className="h[136px] w-full px-4 flex flex-col gap-[24px] justify-center items-center text-center">
          <h1 className="w-[768px] h-[67px] font-[700] text-[56px] font-roboto leading-[67.2px] text-center ">
            Courses
          </h1>
          <p className="w-[768px] h-[27px] font-[400] text-[18px] font-roboto leading-[27px] text-center ">
            Your Ultimate Guide to learning
          </p>
        </div>
        <div className="flex flex-col items-center w-[1280px] h-[1340px] gap-[64px] ">
          <div className="flex w-[336px] h-[40px] mx-[472] ">
            <div className="flex flex-col justify-between items-center text-center w-[87px] h-[40px] gap-[8px] border-b-[1px] border-solid border-[#676767] ">
              Popular
            </div>
            <div className="flex flex-col justify-between items-center text-center w-[140px] h-[40px] gap-[8px]">
              Recommended
            </div>
            <div className="flex flex-col justify-between items-center text-center w-[109px] h-[40px] gap-[8px]">
              Best Price
            </div>
          </div>
          <div className="flex flex-row w-[1280px] h-[1132px] gap-[64px]">
            <div className="flex flex-row w-[1312px] h-[534px] gap-[32px]">
              <div className="flex flex-col w-[380px] h-[534px] gap-[24px] bg-[#F7F7F7]">
                <div className="w-[380px] h-[300px]">
                  <Image
                    src={"/card1.png"}
                    alt=""
                    width={380}
                    height={300}
                  />
                </div>
                <div className="flex flex-col w-[380px] h-[210px] gap-[24px]">
                  <div className="flex flex-col w-[382px] h-[122px] gap-[8px] pl-6">
                    <div className="flex items-center justify-between h-[24px] w-full">
                      <p className="font-[600] text-[14px] leading-[21px] font-roboto">
                        Design
                      </p>
                      <Image
                        className="w-[48px] h-[24px] mr-6"
                        src={"/Star.png"}
                        alt=""
                        width={48}
                        height={24}
                      />
                    </div>
                    <h5 className="w-full font-roboto font-[700] text-[24px] leading-[33.6px]">
                      UX/UI Design 201
                    </h5>
                    <p className="w-full font-roboto font-[400] text-[16px] leading-[24px]">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros.</p>
                  </div>
                  <div className="flex flex-row justify-between w-[382px] h-[40px] gap-[16px] ml-6 ">
                    <div className="flex w-[117px] h-[40px] gap-[8px] rounded-[5px] border-[1px] border-solid border-[#000000] ">
                      <button className="font-[400] text-[16px] leading-[24px] font-roboto justify-center items-center text-center w-full h-full ">Enroll Now</button>
                    </div>
                    <div className="roundede-[5px] gap-[8px]   ">
                    <p className="leading-[24px] font-[500] font-roboto text-[16px] w-[37px] h-[24px] justify-between mr-52 mt-2">$400</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="flex flex-row w-[1312px] h-[534px] gap-[32px]">
              <div className="flex flex-col w-[380px] h-[534px] gap-[24px] bg-[#F7F7F7]">
                <div className="w-[380px] h-[300px]">
                  <Image
                    src={"/card2.png"}
                    alt=""
                    width={380}
                    height={300}
                  />
                </div>
                <div className="flex flex-col w-[380px] h-[210px] gap-[24px]">
                  <div className="flex flex-col w-[382px] h-[122px] gap-[8px] pl-6">
                    <div className="flex items-center justify-between h-[24px] w-full">
                      <p className="font-[600] text-[14px] leading-[21px] font-roboto">
                      Programmimg
                      </p>
                      <Image
                        className="w-[48px] h-[24px] mr-6"
                        src={"/Star.png"}
                        alt=""
                        width={48}
                        height={24}
                      />
                    </div>
                    <h5 className="w-full font-roboto font-[700] text-[24px] leading-[33.6px]">
                    Introduction to Python
                    </h5>
                    <p className="w-full font-roboto font-[400] text-[16px] leading-[24px]">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros.</p>
                  </div>
                  <div className="flex flex-row justify-between w-[382px] h-[40px] gap-[16px]  ml-6">
                    <div className="flex w-[117px] h-[40px] gap-[8px] rounded-[5px] border-[1px] border-solid border-[#000000] ">
                      <button className="font-[400] text-[16px] leading-[24px] font-roboto justify-center items-center text-center w-full h-full ">Enroll Now</button>
                    </div>
                    <div className="roundede-[5px] gap-[8px]   ">
                    <p className="leading-[24px] font-[500] font-roboto text-[16px] w-[37px] h-[24px] justify-between mr-52 mt-2">$400</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="flex flex-col w-[1312px] h-[534px] gap-[32px]">
              <div className="flex flex-col w-[380px] h-[534px] gap-[24px] bg-[#F7F7F7]">
                <div className="w-[380px] h-[300px]">
                  <Image
                    src={"/card3.png"}
                    alt=""
                    width={380}
                    height={300}
                  />
                </div>
                <div className="flex flex-col w-[380px] h-[210px] gap-[24px]">
                  <div className="flex flex-col w-[382px] h-[122px] gap-[8px] pl-6">
                    <div className="flex items-center justify-between h-[24px] w-full">
                      <p className="font-[600] text-[14px] leading-[21px] font-roboto">
                      Business
                      </p>
                      <Image
                        className="w-[48px] h-[24px] mr-6"
                        src={"/Star.png"}
                        alt=""
                        width={48}
                        height={24}
                      />
                    </div>
                    <h5 className="w-full font-roboto font-[700] text-[24px] leading-[33.6px]">
                    Data Analysis for Beginners
                    </h5>
                    <p className="w-full font-roboto font-[400] text-[16px] leading-[24px]">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros.</p>
                  </div>
                  <div className="flex flex-row justify-between w-[382px] h-[40px] gap-[16px]  ml-6 ">
                    <div className="flex w-[117px] h-[40px] gap-[8px] rounded-[5px] border-[1px] border-solid border-[#000000] ">
                      <button className="font-[400] text-[16px] leading-[24px] font-roboto justify-center items-center text-center w-full h-full ">Enroll Now</button>
                    </div>
                    <div className="roundede-[5px] gap-[8px]   ">
                    <p className="leading-[24px] font-[500] font-roboto text-[16px] w-[37px] h-[24px] justify-between mr-52 mt-2">$400</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="flex flex-row w-[1280px] h-[1132px] gap-[64px]">
            <div className="flex flex-row w-[1312px] h-[534px] gap-[32px] ">
              <div className="flex flex-col w-[380px] h-[534px] gap-[24px] bg-[#F7F7F7]">
                <div className="w-[380px] h-[300px]">
                  <Image
                    src={"/card5.png"}
                    alt=""
                    width={380}
                    height={300}
                  />
                </div>
                <div className="flex flex-col w-[380px] h-[210px] gap-[24px]">
                  <div className="flex flex-col w-[382px] h-[122px] gap-[8px] pl-6">
                    <div className="flex items-center justify-between h-[24px] w-full">
                      <p className="font-[600] text-[14px] leading-[21px] font-roboto">
                      Art
                      </p>
                      <Image
                        className="w-[48px] h-[24px] mr-6"
                        src={"/Star.png"}
                        alt=""
                        width={48}
                        height={24}
                      />
                    </div>
                    <h5 className="w-full font-roboto font-[700] text-[24px] leading-[33.6px]">
                    Art Specialization
                    </h5>
                    <p className="w-full font-roboto font-[400] text-[16px] leading-[24px]">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros.</p>
                  </div>
                  <div className="flex flex-row justify-between w-[382px] h-[40px] gap-[16px]  ml-6">
                    <div className="flex w-[117px] h-[40px] gap-[8px] rounded-[5px] border-[1px] border-solid border-[#000000] ">
                      <button className="font-[400] text-[16px] leading-[24px] font-roboto justify-center items-center text-center w-full h-full ">Enroll Now</button>
                    </div>
                    <div className="roundede-[5px] gap-[8px]   ">
                    <p className="leading-[24px] font-[500] font-roboto text-[16px] w-[37px] h-[24px] justify-between mr-52 mt-2">$400</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="flex flex-row w-[1312px] h-[534px] gap-[32px]">
              <div className="flex flex-col w-[380px] h-[534px] gap-[24px] bg-[#F7F7F7]">
                <div className="w-[380px] h-[300px]">
                  <Image
                    src={"/card5.png"}
                    alt=""
                    width={380}
                    height={300}
                  />
                </div>
                <div className="flex flex-col w-[380px] h-[210px] gap-[24px]">
                  <div className="flex flex-col w-[382px] h-[122px] gap-[8px] pl-6">
                    <div className="flex items-center justify-between h-[24px] w-full">
                      <p className="font-[600] text-[14px] leading-[21px] font-roboto">
                      Law
                      </p>
                      <Image
                        className="w-[48px] h-[24px] mr-6"
                        src={"/Star.png"}
                        alt=""
                        width={48}
                        height={24}
                      />
                    </div>
                    <h5 className="w-full font-roboto font-[700] text-[24px] leading-[33.6px]">
                    Rule of Law
                    </h5>
                    <p className="w-full font-roboto font-[400] text-[16px] leading-[24px]">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros.</p>
                  </div>
                  <div className="flex flex-row justify-between w-[382px] h-[40px] gap-[16px]  ml-6">
                    <div className="flex w-[117px] h-[40px] gap-[8px] rounded-[5px] border-[1px] border-solid border-[#000000] ">
                      <button className="font-[400] text-[16px] leading-[24px] font-roboto justify-center items-center text-center w-full h-full ">Enroll Now</button>
                    </div>
                    <div className="roundede-[5px] gap-[8px]   ">
                    <p className="leading-[24px] font-[500] font-roboto text-[16px] w-[37px] h-[24px] justify-between mr-52 mt-2">$400</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="flex flex-col w-[1312px] h-[534px] gap-[32px]">
              <div className="flex flex-col w-[380px] h-[534px] gap-[24px] bg-[#F7F7F7]">
                <div className="w-[380px] h-[300px]">
                  <Image
                    src={"/card6.png"}
                    alt=""
                    width={380}
                    height={300}
                  />
                </div>
                <div className="flex flex-col w-[380px] h-[210px] gap-[24px]">
                  <div className="flex flex-col w-[382px] h-[122px] gap-[8px] pl-6">
                    <div className="flex items-center justify-between h-[24px] w-full">
                      <p className="font-[600] text-[14px] leading-[21px] font-roboto">
                      Tech
                      </p>
                      <Image
                        className="w-[48px] h-[24px] mr-6"
                        src={"/Star.png"}
                        alt=""
                        width={48}
                        height={24}
                      />
                    </div>
                    <h5 className="w-full font-roboto font-[700] text-[24px] leading-[33.6px]">
                    Introduction to webflow
                    </h5>
                    <p className="w-full font-roboto font-[400] text-[16px] leading-[24px]">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse varius enim in eros.</p>
                  </div>
                  <div className="flex flex-row justify-between w-[382px] h-[40px] gap-[16px]  ml-6 ">
                    <div className="flex w-[117px] h-[40px] gap-[8px] rounded-[5px] border-[1px] border-solid border-[#000000] ">
                      <button className="font-[400] text-[16px] leading-[24px] font-roboto justify-center items-center text-center w-full h-full ">Enroll Now</button>
                    </div>
                    <div className="roundede-[5px] gap-[8px]   ">
                    <p className=" leading-[24px] font-[500] font-roboto text-[16px] w-[37px] h-[24px] justify-between mr-52 mt-2  ">$400</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="flex justify-center items-center text-center w-[152px] h-[70px] rounded-[5px] border-[1px] border-[#000000] gap-[8px] ">
            <button className="w-[120px] h-[24px] leading-[24px] text-[16px]font-[400] font-roboto ">View All Courses</button>
          </div>
        </div>
      </section>
      </div>
  );
};

export default CoursesCards;